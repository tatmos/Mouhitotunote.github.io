# UIサイズ調整まとめ - 960x540対応

## 修正完了

Reference Resolutionを960x540に変更した後、UIが画面からはみ出していた問題を、ボタンやUI要素のサイズを調整することで解決しました。
...
- `.scenario-grid-container`: `max-width: 960px` → **720px**
- `.profile-tab-layout`: `min-height: 500px` → **400px**
- `.profile-list-container`: `width: 250px` → **200px**
- `.profile-list-button`: `min-height: 60px` → **48px**, `padding: 12px` → **10px**, `font-size: 16px` → **14px**
- `.profile-card`: `width: 300px` → **240px**, `min-height: 380px` → **300px**, `padding: 15px` → **12px**, `margin: 10px` → **8px**
- `.profile-detail-card`: `padding: 20px` → **15px**, `min-height: 400px` → **320px**

#### フォントサイズ
- `.title-text`: `font-size: 48px` → **36px**
- `.title-text-dark`: `font-size: 48px` → **36px**
- `.score-text`: `font-size: 24px` → **18px**
- `.score-text-anomaly`: `font-size: 24px` → **18px**
- `.word-get-success`: `font-size: 20px` → **16px**, `padding: 16px` → **12px**
- `.word-get-dark`: `font-size: 20px` → **16px**, `padding: 16px` → **12px**
- `.word-get-failed`: `padding: 16px` → **12px**
...
960x540は1920x1080のちょうど半分なので、基本的にサイズを約70-80%に調整しました：

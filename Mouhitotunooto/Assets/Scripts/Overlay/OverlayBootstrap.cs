...
namespace NovelGame.Overlay
{
    ...
    public class OverlayBootstrap : MonoBehaviour
    {
        ...
        private IEnumerator DelayedInitialization()
        {
            ...
            if (overlayDocument != null)
            {
                overlayDocument.sortingOrder = 30;
        ...
        public void DebugForcePhaseToActive()
        {
            if (state != null && presenter != null)
            {
                ...
                presenter.UpdatePhase(state.CurrentPhase);
                Debug.Log("[OverlayBootstrap] PhaseをActiveに設定しました");
            }
            else
            {
                Debug.LogError("[OverlayBootstrap] stateまたはpresenterがnullです");
            }
        }
        ...
        public void DebugTestSpeechSystem()
        {
            ...
            if (presenter is OverlayPresenter_UITK uiPresenter)
            {
                ...
                if (balloonRoot != null && balloonLabel != null)
                {
                    ...
                    Debug.Log("[OverlayBootstrap] テストセリフを表示しました");
                }
                else
                {
                    Debug.LogError("[OverlayBootstrap] セリフ表示要素が見つかりません");
        ...
        public void DebugResetReactionHistory()
        {
            ...
            if (state is OverlayState overlayState)
            {
                ...
                Debug.Log("[OverlayBootstrap] リアクション発動履歴をリセットしました");
            }
        }
            ...
            if (overlayDocument != null)
            {
                ...
                if (root != null)
                {
                    ...
                    if (overlayRoot != null)
                    {
                        ...
                        Debug.Log($"[OverlayBootstrap] UIDocument状態: enabled={overlayDocument.enabled}, Phase={state?.CurrentPhase}");
                        Debug.Log($"[OverlayBootstrap] root: {root.resolvedStyle.width}x{root.resolvedStyle.height}");
                        Debug.Log($"[OverlayBootstrap] GirlImage: {(girlImage != null ? $"{girlImage.resolvedStyle.width}x{girlImage.resolvedStyle.height}" : "null")}");
                        Debug.Log($"[OverlayBootstrap] RoomImage: {(roomImage != null ? $"{roomImage.resolvedStyle.width}x{roomImage.resolvedStyle.height}" : "null")}");
                    }
                }
            }
            else
            {
                Debug.LogError("[OverlayBootstrap] overlayDocument is null!");
            }
        }

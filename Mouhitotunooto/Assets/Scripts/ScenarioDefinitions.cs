...
namespace NovelGame
{
    ...
    public static class ScenarioDefinitions
    {
        ...
        {
            // 本日のおすすめのメニュー名を取得（保存された値を使用）
            string todayRecommendation = RestaurantMenuManager.GetMenuName();
            ...
            else if (loopCount >= 2)
            {
                ...
                scenario.branches[2] = new Branch
                {
                    text = "シェフ：「お待たせしました」\n運ばれてきたのは空の皿。\n私：「あの・・・これは?」\nシェフ：にっこり笑って「今日のきまぐれは『想像の料理』です」\n私：「・・・?」\nシェフ：「冗談です。実は、きまぐれメニューは今日はお休みなの」\nシェフ：「もう、ひとつも残っていないの。ごめんね」\n\nあなたは少しがっかりしたが、シェフのユーモアに笑顔を浮かべる。",
                    hasWord = true,
                    epilogue = hasCollectedU ? "シェフの言葉から、レストランの秘密を知ることができました。\n「もう、ひとつ」という言葉に隠された意味を感じ取りました。\n" + MouhitotsuWordManager.GetLetterAcquiredEpilogue('う', 2, loopCount, false) : "シェフの言葉から、レストランの秘密を知ることができました。\n「もう、ひとつ」という言葉に隠された意味を感じ取りました。",
                    epilogue2 = hasCollectedU ? MouhitotsuWordManager.GetLetterAcquiredEpilogue2('う', 2, loopCount, false, "シェフの言葉") : string.Empty,
        ...
        private static Scenario CreateScenario3(int loopCount = 1)
        {
            var scenario = new Scenario
            {
                ...
                {
                    new Choice { id = 1, text = "一人で開ける", preview = $"中には手紙と{TimeCapsuleItemManager.GetTimeCapsuleItem()}..." },
        ...
        {
            // ランダムな動物名を取得（保存された値を使用）
            string animalName = AnimalNameManager.GetAnimalName();
